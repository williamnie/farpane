...
H5.3q 新增 raw-only `Scripts/sample-farpane-host-powermetrics.py`：只接受 `battery-idle|battery-active`、absolute output prefix 与 exact Host PID；acceptance 为 600–1,800 秒，smoke 为 1–60 秒。脚本自身不调用 `sudo`，必须由操作者在便携 Mac 的验收终端中显式以 root 启动，例如 `sudo FARPANE_HOST_POWER_SAMPLE_MODE=smoke /absolute/repo/Scripts/sample-farpane-host-powermetrics.py battery-idle 10 /absolute/evidence/prefix <HOST_PID>`。输出 `<prefix>.powermetrics.pliststream` 与 `<prefix>.powermetrics.json`，拒绝不可信目录、symlink、覆盖、进程/可执行文件漂移、AC start/end、超时和超限；metadata 固定声明 raw 未解析、全窗 battery/阈值/thermal response 未证明、item 9 未完成。真实 parser 必须基于 portable-Mac raw fixture 实现，不得从 man page 或猜测字段名伪造 schema。
