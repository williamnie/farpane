...
> 更新（2026-08-08）：**H4.3c1 process-lifetime XPC identity authority 已完成自动实现**。审计确认 bootstrap build ID 在 Host 启动前可用，但 Host instance ID 只有 initial authoritative snapshot 后出现，且此前没有 agentBootId owner；因此新增独立 authority，在 process composition 时校验 build ID 并只生成一次 lowercase canonical UUID boot ID，初态等待 Host。首次合法 Host token 绑定为 immutable ready identity；同值并发/重入幂等，不同或非法 Host identity、显式 termination 均永久 invalidated，失效后不可恢复。64-way same-ID concurrency 验证精确 1 bound/63 unchanged。authority 不读取 Bundle/env/UserDefaults/文件/peer，不持有 XPC/runtime surface。本步尚未接入禁用的 Agent process、snapshot state 或 listener；H4.2k 继续全拒绝。未修改 Host Control/Media ABI、Rust/Hermes/SMAppService/根配置，未安装/部署/push。详见 `Evidence/HostMode/2026-08-08/h4-xpc-process-identity-authority.md`。
