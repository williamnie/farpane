...
> 更新（2026-08-10）：**H6.2k3 Host↔Viewer image transfer wiring contract 已完成自动实现**。Host Control ABI 从 v14 升至 v15，在既有小文本/富文本四方向之外新增互相独立、默认均为 false 的 image read/write；Rust-owned `NativeClipboardTransferPolicy` 保存三类格式策略，只把任一显式方向的并集映射到 pinned upstream 单一 negotiation Boolean，因此 small/rich opt-in 不会隐式开放图片。Host 两个真实 data-plane 方向都先应用 active-session directional revoke，再要求 exactly one RGBA/PNG/SVG 通过 H6.2k1 有界 envelope 与 matching image policy；多图片、畸形 metadata、未知/special format、错误方向与仅 rich policy 全部 fail closed。入站在调用 pinned `update_clipboard` 前、出站在进入 connection writer 前都重建 canonical uncompressed image；RGBA 保留已验证尺寸，PNG/SVG protobuf 尺寸保持零，SVG 仍不是渲染 sanitizer。Viewer ABI v8 的形状与 Host wire shape 一致。当前 Viewer AppKit owner、Viewer 产品配置、Host Agent bootstrap/Home 与 legacy Host 产品调用都没有开启 image，Core 默认也全关，因此本步只建立传输能力，不读取/写入真实 pasteboard、不形成图片产品能力，也未安装/启动 App/Agent、修改 Hermes 或 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-host-viewer-image-transfer-wiring.md`；下一自动边界为 **viewer-image-pasteboard-owner-explicit-enablement-contract**，之后再做 Host image bootstrap/Home opt-in 与安装态双机验收。

> 更新（2026-08-10）：**H6.2k4 Viewer image pasteboard owner and explicit enablement contract 已完成自动实现**。FarPane 的设备连接、自动恢复和环境 live mode 三个真实 Viewer 配置入口现在显式开启 ABI v8 的 image receive/send，Core 默认值仍保持 false。既有唯一 AppKit owner 扩展为六方向 owner，继续只在 authenticated/streaming session epoch 激活、只在 `changeCount` 变化后读取 exactly one pasteboard item，并复用 owned-write suppression、125 ms→4 s 动态退避与 terminal/Home/App teardown。图片优先级为 `public.svg-image`、PNG、TIFF：SVG/PNG 进入既有 canonical policy；TIFF 先受 128 MiB 输入上限和尺寸/像素门禁约束，再转为 canonical PNG；远端 RGBA 同样转换为有界 PNG 写入，远端 PNG/SVG 保留各自标准 pasteboard type。发现图片 type 但内容畸形时整项 fail closed，不降级成 rich/plain 发送；SVG 仍只作为不可信字节传输，不声明渲染 sanitizer。远端 image callback 复用 core generation、attempt 与 clipboard session epoch 三重门禁，旧 Core/recovery gap 回调不能写入当前 pasteboard。Host Agent bootstrap/Home 和 legacy Host 产品调用仍未开启 image，因此本步只完成 Viewer 产品侧 owner，不形成默认开放的端到端图片剪贴板，也未安装/启动 App/Agent、修改 Hermes 或 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-viewer-image-pasteboard-owner-explicit-enablement.md`；下一自动边界为 **host-image-bootstrap-home-opt-in-contract**，之后再做安装态双机 RGBA/PNG/SVG、ownership、teardown、延迟与 idle CPU 验收。

> 更新（2026-08-10）：**H6.2k5 Host image bootstrap and Home opt-in contract 已完成自动实现**。Host Agent immutable bootstrap 从 schema v3 升至 v4，在既有小文本与富文本四方向之外新增互相独立的 `allowRemoteImageRead`/`allowRemoteImageWrite`；v1 仍迁移为六方向全关，v2 保留小文本并把富文本/图片迁移为关闭，v3 保留小文本/富文本并把图片迁移为关闭，future schema、额外键、partial policy 与数值伪装 Boolean 继续 fail closed。六方向 policy 共同参与 canonical projection equality，因此图片任一方向变化也会推进 `configRevision` 并原子发布。Home 增加“允许远端读取本机图片”和“允许远端写入图片到本机”，显示 RGBA/PNG 128 MiB 与 SVG 4 MiB 上限；六项偏好缺失均默认 false，只能在 Host 关闭、控制状态 coherent 且无 Viewer 启动中的情况下修改，修改后立即重新发布 bootstrap。后台 HostAgent 与 legacy 前台 Host 都把同一六方向策略交给 Host Control ABI v15；Core 默认值继续全关，只有用户逐方向显式开启后，既有 Viewer owner 与 Host data-plane 才形成端到端图片能力。本步没有安装/启动 App/Agent、读写真实 pasteboard、修改 Hermes 或 push；SVG 仍不是渲染 sanitizer，文件 promise 仍未实现，双机 ownership/teardown、延迟与 idle CPU 也未验收。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-host-image-bootstrap-home-opt-in.md`；下一人工边界为 **host-image-clipboard-installed-two-mac-acceptance**，设备不可用期间可继续推进 H6.3 文件传输的安全合同。

> 更新（2026-08-10）：**H6.3a Host file-transfer explicit-policy ABI seam 已完成自动实现**。Host Control ABI 从 v15 升至 v16，`RdnHostCreateOptions` 与 Swift `HostServerConfiguration` 新增独立 `enable_file_transfer`/`fileTransferEnabled`，Swift 默认值保持 false；Rust 在 create 时复制 immutable policy，并在任何 identity/network runtime 创建前把 `enable-file-transfer` 精确持久化为 `Y` 或 `N`，随后从 private options 文件按同一 policy readback，缺失或陈旧值继续 fail closed。pinned upstream 的 dedicated `FileTransfer` login 已消费同一个 `OPTION_ENABLE_FILE_TRANSFER`，关闭时会在建立 file-transfer scope 前拒绝。音频仍无条件持久化为 `N`；现有前台 App 与后台 Agent 调用均不传入 file opt-in，因此产品行为仍关闭，也没有新增 Home 开关、Viewer file ABI/UI 或 clipboard file promise。本步只建立“默认关闭、未来可显式开启”的 Host ABI seam，不宣称端到端文件传输或双机验收完成。设备目标已收敛为自动开发与 Mac mini 单机 smoke，双机现场验收记录为未验证但不阻塞后续开发。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-explicit-policy-abi.md`；下一自动边界为 **host-file-transfer-security-boundary-audit**，在任何产品启用前审计路径穿越、symlink、文件名、大小与写入落点。

> 更新（2026-08-10）：**H6.3b Host file-transfer security boundary audit 已完成自动审计**。pinned `hbb_common::fs` 已对相对文件名拒绝 NUL、`..` 与绝对路径，整批 file list 任一项非法即拒绝，并以 `symlink_metadata` 拒绝当前已存在的 symlink 组件；dedicated `FileTransfer` 登录还在建立 scope 前消费 H6.3a 的独立权限，authorized message allowlist 只允许 file action/response。审计同时证明当前不能开放产品：压缩 `FileTransferBlock` 仍调用无界 `decompress`，没有 decoded block 硬上限；写入前的 path check 与 `File::create` 之间明确存在 symlink TOCTOU；remove/create/rename 的绝对操作路径只拒绝 empty/NUL，没有 FarPane-owned destination root；Native Host 为避免旧 CM 多开会丢弃 external CM receiver，但 receive/remove/create/rename 等真实文件操作仍只发送到该 CM channel，所以即使 ABI opt-in 为 true，Native Host 也没有完整 file-service owner。App/Agent 继续不传 opt-in，Release 也不编译 `unix-file-copy-paste`，因此以上缺口保持 fail closed。本步没有改 Rust 行为、ABI/schema/UI、文件系统或产品开关，也不宣称路径 race、payload 或功能链已完成。机器审计与验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-security-boundary.md`；下一自动边界为 **host-file-transfer-bounded-block-envelope**，先消除远端压缩块的无界内存增长，再分别处理 descriptor-based safe open/destination root 与 Native Host file-service owner。

> 更新（2026-08-10）：**H6.3c Host bounded file-transfer block envelope 已完成自动实现**。pinned file sender 与 receiver 现在共用 `128 KiB` canonical block 上限；receiver 在进入 zstd 解压、`data_source` 分支或当前 block 触发的目标文件 create 之前先拒绝 wire payload 超限。压缩块改用带同一 `128 KiB` decoded hard limit 的 bounded helper，raw payload、compressed wire payload、decoded payload 超限以及 malformed compressed input 全部 fail closed；恰好达到 decoded 上限的 raw/compressed payload 保持兼容。实现与 focused tests 由独立 canonical patch 纳入 bootstrap 的 apply/reverse-check inventory，避免只修改工作树而无法重放。App/Agent 仍不传 file opt-in，产品能力继续关闭；本步不改变 resume offset 可能预先打开 stream 的既有生命周期，不关闭 symlink TOCTOU、不定义 FarPane-owned destination root，也不实现 Native Host file-service owner 或 Viewer UI。双机验收因当前只有 Mac mini 记为未验证但不阻塞开发。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-bounded-block-envelope.md`；下一自动边界为 **host-file-transfer-safe-open-root**。

> 更新（2026-08-10）：**H6.3d1 Host descriptor-relative receive-root primitive 已完成自动实现**。新增实现只由 `rdn-native-host` 的 macOS Host bridge 编译，不改变无 Host feature 或其他平台的上游行为。root admission 从 `/` 的真实目录 descriptor 开始逐组件 `openat(O_DIRECTORY|O_NOFOLLOW)`，祖先必须由 root/当前 euid 所有且不可 group/world writable，最终 FarPane-owned receive root 必须是当前 euid 的精确 `0700` 目录；后续操作只持有该 root descriptor，不再依赖可被换掉的原路径。相对目标拒绝 empty、absolute、NUL、`.`、`..`、重复/尾随分隔符；嵌套目录用 `mkdirat/openat` 创建并验证为当前 euid `0700`，新 staging file 用 `O_CREAT|O_EXCL|O_NOFOLLOW` 建立为 `0600`，resume 只接受同 root 下 `O_NOFOLLOW` 打开的当前 euid、`0600`、single-link regular file。测试证明 admission 后把原 root path 重命名并替换为指向外部的 symlink，create 仍写入已固定的原 root inode。该 primitive 尚未接入 Native Host file-service owner，也没有实现 descriptor-relative remove/rename；旧 CM path API 与 upstream `TransferJob` 仍保持原行为，App/Agent 仍不 opt in，产品能力继续关闭。本步没有 ABI/schema/UI、真实用户文件、安装、启动、Hermes 或 push 变更。机器审计与完整验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-safe-receive-root.md`；下一自动边界为 **host-file-transfer-safe-root-mutations**，随后才建立 Native Host file-service owner。

> 更新（2026-08-10）：**H6.3d2 Host descriptor-relative safe-root mutations 已完成自动实现**。同一 admitted root descriptor 现在支持 `mkdirat` 创建精确 `0700` 目录、只删除当前 euid/`0600`/single-link regular file、只删除当前 euid/`0700` 的空目录，以及先校验 source 再用 macOS `renameatx_np(RENAME_EXCL)` 做不可覆盖 rename。全部 lookup 都从已固定的 root/parent descriptor 出发并以 `AT_SYMLINK_NOFOLLOW` 或 `O_NOFOLLOW` 拒绝 symlink；hard link、类型混淆、宽权限 source、非空目录和已存在 destination 均 fail closed。测试还证明 admission 后原 root path 被替换为外部 symlink 时，create/remove/rename 仍只作用于原 root inode。递归删除刻意不提供，避免把未经枚举/确认的远端请求扩展为广泛破坏操作。该 primitive 仍未接入 Native Host file-service owner，旧 CM path API 与 upstream `TransferJob` 未切换，App/Agent 继续不 opt in，因此产品文件传输保持关闭；本步没有 ABI/schema/UI、真实用户文件、安装、启动、Hermes、push 或双机验收。机器审计与验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-safe-root-mutations.md`；下一自动边界为 **host-file-transfer-native-service-owner**。

> 更新（2026-08-10）：**H6.3e1 Native Host file-service owner core 已完成自动实现**。新增 `NativeHostFileServiceOwner` 成为该 macOS Host-only module 唯一对外 authority，持有 H6.3d1 admitted descriptor 并统一代理安全 create/resume/create-directory/remove/rename；底层 `NativeFileTransferRoot` 与全部 primitives 已收口为 module-private，后续 connection 不能绕开 owner 直接操作文件系统。owner 的 `remove_directory(relative, recursive)` 在 `recursive=true` 时于任何 lookup/mutation 前返回固定 `RecursiveRemovalUnsupported`，测试证明包含文件的目录完全未被触碰；非递归空目录仍复用 H6.3d2 的 descriptor-relative contract。owner core 尚未接收 Host 配置中的 receive-root，也未接管 `Connection::send_fs`，因此没有 write-job 生命周期、远端响应、Viewer destination/progress UI 或端到端文件传输；App/Agent 继续不 opt in，产品能力保持关闭。本步不改 ABI/schema/UI、真实用户文件、安装、启动、Hermes、push 或双机验收。机器审计与验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-native-owner-core.md`；下一自动边界为 **host-file-transfer-receive-root-config-contract**，先让显式 opt-in 与安全 root 形成 immutable/fail-closed 配置，再接 connection dispatch。

> 更新（2026-08-10）：**H6.3e2 Host immutable receive-root config contract 已完成自动实现**。Host Control ABI 从 v16 升至 v17，`RdnHostCreateOptions` 新增 create 期间复制的 `file_transfer_receive_root`，Swift `HostServerConfiguration.fileTransferReceiveRoot` 默认 nil 并投影为空 C string。Rust 要求 permission/root 精确成对：关闭只能配空 root，开启必须配非空 root；pair mismatch 在 process singleton claim 前返回 validation。claim 成功后，root 必须立即通过 H6.3d1 descriptor/no-follow/current-euid/`0700` admission，失败时释放 singleton claim 并返回 storage failure；成功 owner 由 `RdnHost` 持有至 destroy，因此 runtime 后续只依赖 admitted descriptor，不重新解析字符串路径。非 macOS 显式开启返回 not-supported。当前 App/Agent 仍构造默认 false/nil，connection 也尚未 dispatch 到 owner，产品文件传输继续关闭；本步没有 Home/Viewer file UI、真实用户文件、安装、启动、Hermes、push 或双机验收。机器审计与验证见 `Evidence/HostMode/2026-08-10/h6-host-file-transfer-receive-root-config-contract.md`；下一自动边界为 **host-file-transfer-connection-mutation-dispatch**，先接安全 create-dir/remove/rename 与稳定远端响应，再处理 write-job lifecycle。
...
> 更新（2026-08-11）：**H6.3e4c Native Host existing-target decision lifecycle 已完成自动实现**。Host 只通过 pinned receive-root descriptor 以 `O_RDONLY|O_NOFOLLOW` 打开已有目标，并要求当前 euid、精确 `0600`、single-link regular file；不安全类型、权限、owner 或 lookup 全部 fail closed。existing target 的真实 size/mtime 与 exact identical 结果通过 upstream `FileTransferDigest(is_upload=true)` 返回发送端，job 在收到决定前拒绝 block。明确 `Skip(true)` 会保留原目标、descriptor-relative 清除安全的旧 staging，并把 skipped expected size 纳入单/多文件完成记账；`OffsetBlk`、`Skip(false)` 与 malformed 决定均作为 replace 请求被固定拒绝，job abort，原文件不变，最终提交仍只允许 `RENAME_EXCL` no-replace。独立 canonical connection patch 已进入 bootstrap apply/reverse-check；3 项 real-filesystem focused tests 覆盖等待门禁、旧 partial 清理、单/多文件 skip、replace 与不安全目标拒绝。App/Agent 继续不传 file opt-in，产品能力保持关闭；read/list/download、Viewer destination/progress UI、多文件 resume 与真实 replace 仍未实现。双机验收按当前目标记为未验证但不阻塞开发；本步不改 ABI/schema/UI、真实用户文件、Hermes、CI、依赖或数据库。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-native-existing-target-lifecycle.md`；下一自动边界为 **host-file-transfer-native-read-list-download-lifecycle**。

> 更新（2026-08-11）：**H6.3e5a Native Host descriptor-relative read/list snapshot primitive 已完成自动实现**。同一 Native Host file-service owner 现在能从 pinned receive-root descriptor 列出 root/相对目录、递归快照下载文件并按快照安全重开。目录读取通过 `openat(dirfd, ".", O_DIRECTORY|O_NOFOLLOW)` 创建独立 open-file description，再用 `fdopendir/readdir` 枚举，避免 `dup` 共享目录游标；只接受当前 euid、精确 `0700` 目录与 `0600` single-link regular file，symlink、宽权限、类型混淆和非法 UTF-8 名称全部 fail closed。private `*.farpane-part` staging 始终隐藏且不能直接快照；单目录与整批文件都限制 1,024 entries、1 MiB metadata，递归深度限制 64。快照记录 relative path、device/inode/size/mtime，真正打开时仍走 `openat(O_RDONLY|O_NOFOLLOW)` 并逐项复核，路径替换即拒绝；root 原路径被移动并替换成外部 symlink 后仍只读取已固定 inode。4 项 real-filesystem tests 覆盖重复列目录/hidden/staging、递归与 pinned-root 读取、replacement/symlink/unsafe-mode 拒绝和 entry limit。本步只建立 connection sender 所需的安全只读 primitive，尚未路由 `ReadDir`/`AllFiles`/`Send`、创建 read job 或发送 block/digest/done；产品仍默认关闭，双机验收仍为非阻塞未验证。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-native-read-list-snapshot.md`；下一自动边界为 **host-file-transfer-native-read-list-download-connection-lifecycle**。

> 更新（2026-08-11）：**H6.3e5b Native Host read/list/download connection lifecycle 已完成自动实现**。dedicated `FileTransfer` connection 现在先把虚拟 wire root `/` 的 `ReadDir`、`ReadEmptyDirs` 与 `AllFiles` 路由到 H6.3e5a 的 pinned descriptor owner，完全不存在 Native Host 时才保留上游 CM/path fallback；Host live 但 owner/binding 不可用、非法路径或不安全快照只返回固定且不含路径的 rejected/unavailable。Generic `Send` 由 connection 本地持有最多 8 个 read jobs，并与 native write、上游 read 及 CM job ID 冲突门禁；每个 job 先发 snapshot-bound directory，再按 128 KiB 上限发送可选压缩 block。现代 peer 的 overwrite detection 会在每个文件前发 `Digest(is_upload=false)` 并暂停：`Skip(true)` 跳过，`Skip(false)` 从 0 继续，`OffsetBlk` 只允许不超过快照 size 的 `UInt32` offset，malformed/错 file number/替换后的文件全部终止；文件打开、确认及 EOF 均复核 owner identity 与 device/inode/size/mtime。timer 只轮询非等待 job，确认会重新唤醒；cancel、remote error、connection close 与 Host unbind 会确定性释放 read handle，Done/error 后立即移除 job。独立 canonical connection patch 已进入 bootstrap apply/reverse-check，并通过 clean reverse/forward replay；3 项 real-filesystem read-job tests 与 1 项 wire confirmation mapping test 覆盖列表/空目录、bounded stream、skip/offset、replacement、unbind 与 malformed confirmation。App/Agent 继续不传 file opt-in，Viewer destination/progress API 与产品 UI 尚未建立，因此文件传输仍默认关闭；多文件 upload resume 与 existing-target replace 也仍未实现。双机传输按当前开发目标记为未验证且不阻塞。本步不改 ABI/schema、真实用户文件、Hermes、CI、依赖或数据库。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-native-read-list-download-connection.md`；下一自动边界为 **host-file-transfer-viewer-destination-progress-api-contract**。

> 更新（2026-08-11）：**H6.3f1 Viewer file-transfer destination/progress API contract 已完成自动实现**。CoreBridge 新增纯 Swift、默认未接产品的 download contract：本地落点只以 nonzero opaque destination lease 表示并绑定 exact Viewer session epoch，manifest 把所有远端路径继续视为不可信输入，拒绝 absolute/NUL/empty/`.`/`..`、非 NFC、大小写/规范化别名、祖先类型冲突及 private `*.farpane-part`，同时限制 1,024 entries、1 MiB UTF-8 metadata 并以 checked `UInt64` 汇总大小。progress authority 最多持有 8 个 positive transfer ID，只接纳同 session、严格递增 sequence、单调且不超过 manifest 的 file/byte progress 与 finite nonnegative speed；conflict 必须绑定有效 file number，completion 必须精确达到总文件数/字节数，terminal 后立即移除。local cancel 进入 `cancelling` 后只允许 completed/cancelled/稳定 typed failure 收尾，stale session、duplicate ID、回退/越界/畸形 progress 与 teardown 后事件全部 fail closed；UI snapshot 不含本地 path、descriptor 或 raw protocol error。该步骤冻结未来 Viewer adapter/Core ABI 的 destination、progress、cancel 与 teardown 语义；H6.3f2a/b1/b2a 已在其后增加 ABI seam、dedicated session 与 Rust listing envelope，但文件 descriptor owner、真实 I/O 或产品 UI 仍未实现，App/Agent 也继续不 opt in，产品能力保持关闭。双机文件传输仍明确未验证且不阻塞开发。机器审计与完整验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-destination-progress-contract.md`；下一自动边界为 **host-file-transfer-viewer-list-command-callback-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2a Viewer file-transfer ABI v9 default-off seam 已完成自动实现**。Viewer ABI 从 v8 升至 v9，保留全部剪贴板语义，并新增 path-free scalar `RDNFileTransferEvent` callback、exact `enable_file_transfer`/`file_transfer_session_epoch` config pair、epoch-scoped cancel command 与稳定 client error constants。普通 Viewer 会话使用 false/0；pair mismatch 在任何 runtime mutation 前 validation fail closed。Swift 同步复制并重新验证 callback 的 ABI、positive epoch/ID/sequence、已知 kind/failure、单调上限、finite speed、conflict file number 与 exact completion，disconnect 先关闭 file callback delivery；shim/build 同时把 cancel symbol 纳入 required ABI。H6.3f2b1/b2a 已在其后把 true/nonzero 接到 dedicated file session 并建立内部 listing envelope，但 Rust 仍不产生文件事件、不建立 destination descriptor/list/download event mapping；产品入口未传 opt-in，能力继续关闭。双机文件传输继续记为未验证且不阻塞开发。本步未安装/启动 App、修改 Hermes、CI、依赖或数据库，也未 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-abi-v9-seam.md`；下一自动边界为 **host-file-transfer-viewer-list-command-callback-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b1 Viewer dedicated file-session and cancel dispatch lifecycle 已完成自动实现**。ABI v9 的 true/nonzero pair 现在只选择 upstream `ConnType::FILE_TRANSFER`，并在启动前拒绝任何同时开启的小文本、富文本或图片 desktop clipboard direction；false/0 桌面连接保持原路径。文件会话不会启动视频 stream-configuration/TestDelay housekeeping，不会授予 keyboard/mouse input；认证成功按 `authenticated → file-transfer-ready` 投影，worker 退出与显式 disconnect 都清除 mode/epoch。cancel 必须依次通过 positive ID/epoch、active file mode、exact epoch、authenticated、remote file permission 与 ready session sender，成功只投递 upstream `Data::CancelJob`，其余返回稳定错误；测试读取真实 channel 证明 stale/permission gate 与 exact command。H6.3f2b2a 已在其后增加 Rust listing envelope；当前仍没有 list/download start command、远端 manifest callback、progress/job terminal event mapping、destination descriptor owner 或产品 UI，App/Agent 也未 opt in，因此不会访问本地文件且产品能力保持关闭。双机传输仍为未验证、非阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖或数据库，也未 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-dedicated-session.md`；下一自动边界为 **host-file-transfer-viewer-list-command-callback-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2a Viewer remote-list structural envelope 已完成自动实现**。Viewer Rust 新增尚未接 ABI/product 的 owned listing primitive：空目录列表合法，其他列表最多 1,024 个 entry、聚合 UTF-8 name metadata 最多 1 MiB；只接受 regular `File` 与 size=0 的 `Dir`，拒绝 link/drive/unknown type、hidden、empty/`.`/`..`、separator/control character、private `*.farpane-part` 及 ASCII case alias，所有 name 复制为 Rust-owned `String`。focused tests 证明中文 UTF-8 名称和 ownership 保留，并覆盖 type/name/alias/hidden/staging/entry/metadata 边界。该 primitive 刻意不宣称完整 Unicode NFC/case-fold，未来 Swift manifest callback 必须再按 H6.3f1 重验；当前没有 list command/callback、不会访问 destination 或传输 bytes，ABI 仍为 v9，产品继续关闭。双机传输仍为未验证、非阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖或数据库，也未 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-listing-envelope.md`；下一自动边界为 **host-file-transfer-viewer-list-command-callback-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2b Viewer root-list command/callback ABI lifecycle 已完成自动实现**。Viewer ABI 从 v9 升至 v10，保留既有 clipboard、file-session、progress/cancel 语义，新增 callback-scoped `RDNFileTransferListEvent`/entry/status、required `rdn_client_file_transfer_list_root` 与 Swift `requestFileTransferRootList`。命令只接受 exact active file epoch 与 positive request ID，并依次要求 authenticated、remote file permission、ready sender；每个 client 最多一个 pending request，只发送 `ReadDir(path="/", include_hidden=false)`，send failure、disconnect、worker exit 与 `clear_all_jobs` 都释放 pending。远端 root response 必须匹配 nonlocal/full/root shape，随后通过 H6.3f2b2a 的 1,024 entries、1 MiB metadata、regular file/zero-size directory、name/type/ASCII alias 门禁；回调只在当前 active/authenticated/exact epoch 投递，entries/name 仅 callback 期间有效，错误只投影 stable rejected/unavailable，不泄漏远端路径或 raw error。Swift 在回调内复制所有 bytes，并再次验证 ABI/epoch/request/status/count、UTF-8、byte-exact NFC、完整 case-fold collision、separator/control、private staging 与 directory size 后才排队；本步同时修正了 H6.3f1 之前用 Swift 规范等价字符串比较判断 NFC 的漏洞，改为 UTF-8 byte-exact 比较。当前仍没有 recursive manifest/download start、destination descriptor owner、transfer I/O 或产品 UI，App/Agent 不 opt in，能力保持关闭。双机传输继续未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-root-list-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-destination-descriptor-owner**。

> 更新（2026-08-11）：**H6.3f2b2c Viewer destination descriptor owner 已完成自动实现**。Viewer 现在可把用户预先选择的本地目录固定为 exact session epoch 的 opaque destination lease：路径只在初始化时用于 `open(O_RDONLY|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC)`，owner 后续只保存 descriptor、device/inode identity、epoch 与 nonzero token，不保存或对外暴露路径。初始化只接受当前 euid、精确 `0700`、仍有 link 的目录；每次 scoped borrow 都重新 `fstat` 并验证 identity、owner、mode 与类型，stale epoch/token、权限漂移、symlink 或类型混淆全部 fail closed。borrow 锁覆盖 callback，避免 teardown 关闭正在借用的 fd；只有 exact epoch 可 teardown，deinit 也确定性 close。路径被重命名并以 symlink 替换后，borrow 仍绑定原 inode。当前没有 recursive manifest、download start、`openat`/write/mkdir、产品 picker/UI 或 App/Agent opt-in，故不会创建或改写文件，产品能力继续关闭；双机传输仍为未验证且不阻塞。本步不改 ABI、Hermes、CI、依赖、数据库，也未安装/启动 App 或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-destination-descriptor-owner.md`；下一自动边界为 **host-file-transfer-viewer-recursive-manifest-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2d Viewer recursive manifest authority lifecycle 已完成自动实现**。CoreBridge 新增 transport-independent、exact-session/single-flight 的递归清单 authority：positive epoch/request 才能 begin，同一 active request 必须恰好接收一个 files part 与一个 empty-directories part，任意顺序均可；每个 part 在保留前独立执行 1,024 entries 与 1 MiB metadata 上限，空目录还复用 canonical relative-path 门禁。只有两部分都到齐且能通过既有 `ViewerFileTransferManifest` 的 checked total bytes、完整 case-fold collision、ancestor type conflict 与 combined limit 校验时才一次性完成；duplicate/exact-request malformed part 会终止为 stable protocol violation，stale epoch/request 不改变当前状态。远端 rejected/unavailable/connection-closed 和 exact epoch teardown 都是 terminal，local-I/O/protocol failure 不能伪装成 remote status。authority 不持有 destination path/descriptor，也未改 ABI 或接网络；当前仍无 recursive manifest command/callback、download start、文件创建/写入、picker/UI 或产品 opt-in。双机传输继续未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-recursive-manifest-authority.md`；下一自动边界为 **host-file-transfer-viewer-recursive-manifest-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2e Viewer recursive manifest ABI lifecycle 已完成自动实现**。Viewer ABI 升至 v11，在既有 dedicated file-session、root list、destination descriptor owner 与 transport-independent manifest authority 之上新增 exact-session、single-flight 的递归清单 command/callback。命令只发送远端根 `AllFiles(id, "/", include_hidden=false)` 与 `ReadEmptyDirs("/", include_hidden=false)`；Rust 分别将 regular files 和 empty directories 转成 callback-scoped semantic part，在拥有 metadata 后执行每部分 1,024 entries/1 MiB、canonical relative path、hidden/private staging/type/case-alias 门禁。两部分可任意顺序到达；duplicate、malformed、exact remote error、send failure、disconnect、worker exit 与 job teardown 均清除 pending request，stable callback 不暴露 raw protocol error。由于 `ReadEmptyDirsResponse` 没有 request ID，同一 session epoch 只允许一次 manifest request，重试必须以 fresh epoch 重连，避免迟到响应被归到下一请求。Swift 在 queued delivery 前同步复制并复核 ABI/epoch/request/status/part、UTF-8/path/type/size/mtime/case-fold，再投影到 H6.3f2b2d authority。header、shim、build symbol gate 与 built-core contract 同步要求新 symbol。当前仍无 download start、`openat`/write/staging、picker/UI 或 App/Agent opt-in，产品能力保持关闭；双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-recursive-manifest-abi-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-download-start-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2f Viewer download-start ABI lifecycle 已完成自动实现**。Viewer ABI 升至 v12，在 exact-session recursive manifest 完成后新增 path-free queued registration：Swift 只把 session epoch、completed manifest request ID、transfer ID、total files 与 total bytes 投影进 ABI，destination lease/token/path/descriptor 全部留在 Swift owner。Rust 只接受 active dedicated file session、exact epoch、authenticated、remote file permission、ready sender 与 exact completed manifest；最多保留 8 个 unique transfer ID，并拒绝 invalid/duplicate/stale/over-limit 请求。注册成功只建立 connection-local lifecycle ownership，不发送 `Data::SendFiles` 或任何 wire download request，不借 destination descriptor，也不创建 staging/file 或写入字节。cancel 在 upstream `CancelJob` 成功入队后释放 matching registration；job error/done、clear-all、disconnect、connect reset 与 worker exit 也清理状态。header、shim、build symbol gate、Swift scalar projection、Rust regression 与 built-core symbol contract 已同步。App/Agent 仍无 file opt-in、picker/UI 或调用路径，产品能力保持关闭；download dispatch、progress/terminal callback mapping 与 descriptor-relative receive I/O 留给后续独立边界。双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-download-start-abi-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-download-progress-terminal-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2g Viewer download progress/terminal lifecycle 已完成自动实现**。既有 v12 path-free registration 现在把 upstream `job_progress`、`job_done`、`job_error` 与成功 cancel 投影到已有 `RDNFileTransferEvent` callback。每个 job 从 sequence 1 开始严格递增；progress 只接受 finite nonnegative speed、整数且不越界/不回退的 completed bytes，以及单调有界的 completed file number，并按下一未完成文件投影 current file。completion 总是使用 exact manifest totals；cancel 保留最后可靠 progress；remote error 只映射 stable unavailable，不传 raw error。所有 terminal 先移除 registration，callback 在 job mutex 释放后同步发出，并继续受 active/authenticated/file-mode/exact epoch 门禁。Swift 把 callback 解析集中到一个 strict `CoreFileTransferEvent` initializer，复核 ID/sequence、file/byte bounds、finite speed、current-file/terminal shape、typed failure 与 exact completion，再无路径地投影到既有 Viewer progress authority；畸形事件 fail closed。此步没有发送 `Data::SendFiles` 或其他下载 wire request，不借 destination descriptor，不创建 staging/file、不写字节，App/Agent 也仍无 file opt-in/picker/UI，产品能力保持关闭。双机 transfer/progress 未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-download-progress-terminal-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-safe-staging-reservation-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2h Viewer safe staging reservation lifecycle 已完成自动实现**。Swift destination owner 现在只通过 exact-session opaque lease 与既有 pinned root descriptor 为 manifest file 预留新文件 staging：重新验证 canonical relative path、positive transfer ID、0...1,023 file number、unique nonzero reservation token，并限制最多 8 个 active transfer。owner 用 `openat(".", O_DIRECTORY|O_NOFOLLOW)` 取得独立 parent ownership，逐组件只创建或接受当前 euid、精确 `0700` 的真实目录；final name 必须不存在，staging 固定为 manifest 已禁止的 `*.farpane-part`，以 `O_WRONLY|O_CREAT|O_EXCL|O_NOFOLLOW|O_CLOEXEC` 建立并重验为当前 euid、`0600`、single-link、empty regular file。返回 handle 只含 epoch/transfer/file number/token，不含 path/descriptor。cancel、exact teardown 与 deinit 会关闭 owned descriptors，且只有 staging name 仍指向创建时 device/inode 且安全 metadata 未漂移才 unlink；被替换或 hard-link 的 entry 保留并 fail closed。真实文件系统 regression 还证明 root path 被移走并以外部 symlink 替换后，reservation 仍只落在原 pinned inode。此步不写 payload、不 fsync、不 rename/commit final、不发 `Data::SendFiles` 或其他 wire command、不改 ABI/product opt-in/UI，产品能力继续关闭；双机传输未验证且不阻塞。本步只操作 UUID 临时目录，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-safe-staging-reservation-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-safe-receive-write-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2i Viewer safe receive write lifecycle 已完成自动实现**。exact reservation 现在绑定完整 `ViewerFileTransferDownloadRequest` 与 manifest file index，只接受 local `off_t` 可表示的 declared size；payload 必须 nonempty 且单块不超过与 wire envelope 一致的 128 KiB。每块使用 `pwrite` 写到 owner 跟踪的 exact offset，处理 EINTR/partial write，并以 checked `UInt64` 保证 aggregate 永不超过 manifest size。写前和写后都从 parent descriptor 对 staging name 执行 `fstatat(AT_SYMLINK_NOFOLLOW)`，同时 `fstat` open fd，二者必须继续匹配创建时 device/inode、当前 euid、精确 `0600`、single-link regular file和 owner 跟踪的 exact size；invalid/empty/oversize/over-total、metadata/name drift 或 partial/system failure 都会移除 active reservation，并只在 name 仍指向安全原 inode/size 时 unlink partial。达到 declared size 仍不会自动发布 final。真实文件系统测试覆盖多块精确拼接、越界后清 partial、空/超限 block、replacement 不写不删、已写 partial teardown 与不可表示 size admission。此步不 fsync、不设置 mtime、不 rename/commit final，不发 wire request、不改 ABI/product opt-in/UI，产品能力继续关闭；双机传输未验证且不阻塞。本步只操作 UUID 临时目录，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-safe-receive-write-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-safe-receive-commit-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2j Viewer safe receive commit lifecycle 已完成自动实现**。只有 exact handle 仍对应 active reservation、tracked bytes 精确等于 manifest declared size、staging name 与 open fd 仍匹配创建时 device/inode、当前 euid、`0600`、single-link regular file 与 exact size 时才允许 commit。owner 先按既有协议语义把 nonnegative modified time 作为 Unix 秒用 `futimens` 写到 open fd，并执行 `fsync(file)` 后再次复核 identity/metadata；final name 必须仍不存在，随后只通过同一 pinned parent descriptor 的 `renameatx_np(RENAME_EXCL)` 原子发布，永不覆盖竞态出现的 final，最后 `fsync(parent)` 持久化目录项。rename 前任何失败都会终止 reservation，并只删除仍指向原安全 inode 的 staging；rename 已成功但 parent fsync 失败时返回独立 `durabilityUnconfirmed` 终态，保留已发布 final，避免上层把它当作可安全重试的普通拒绝。真实文件系统 regression 覆盖成功内容/mtime/mode、staging 消失、stale handle、未写满拒绝清理及 final race 不覆盖。此步不发 wire download request、不改 ABI/product opt-in/UI，产品能力继续关闭；双机传输未验证且不阻塞。本步只操作 UUID 临时目录，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-safe-receive-commit-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-download-dispatch-receive-adapter-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2k Viewer inbound block envelope 已完成自动实现**。为避免未来 receive callback 把不可信压缩数据直接交给 Swift，本步先在 Rust 建立 path-free owned block 语义封装：入站 `FileTransferBlock` 必须匹配已注册 download job 的 positive transfer ID，file number 必须可转为 nonnegative `u32` 且严格小于该 job 的 manifest total files；wire data 必须 nonempty 且不超过 upstream `MAX_FILE_TRANSFER_BLOCK_BYTES`（128 KiB）。compressed payload 只通过同一 128 KiB 上限的 `decompress_with_limit` 解码，raw payload 也复制为 owned `Vec<u8>`，decoded empty/oversize、malformed compressed、wrong job、negative/out-of-range file number 均 fail closed。Rust regression 覆盖 raw ownership、decoded-limit 压缩成功、wire oversize、解压炸弹、非法压缩和 identity/file bounds。此步不接 io-loop、不新增 receive callback/ABI、不发 `Data::SendFiles`、不访问 Swift descriptor，也不改 product opt-in/UI，能力继续关闭；双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-inbound-block-envelope.md`；下一自动边界为 **host-file-transfer-viewer-receive-block-abi-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2l Viewer receive-block ABI lifecycle 已完成自动实现**。Viewer ABI 从 v12 升至 v13，新增 callback-scoped `RDNFileTransferReceiveBlock` 与 required callback slot，只携带 ABI version、exact session epoch、positive transfer ID、bounded file number 和 decoded byte slice，不跨 ABI 传 path、destination lease 或 descriptor。Rust 只从 H6.3f2b2k 已拥有化且限界验证的 semantic block 构造 raw callback，并在调用前再次要求 active、authenticated、file-transfer mode、exact current epoch、positive ID/global file-number bounds 与 nonempty 128 KiB payload；callback pointer 仅同步调用期间有效。Swift 在 callback 内立即复制 bytes，再以单一 `CoreFileTransferReceiveBlock` initializer 重验 positive epoch/ID、0...1,023 file number、nonempty 与 128 KiB 上限，之后才进入既有 file-transfer lifecycle queue；disconnect/teardown 继续先关闭该共享 delivery gate。Rust regression 覆盖 gated exact-session callback 与 callback-scoped ownership，Swift regression 覆盖 semantic bounds，fresh built-core load 验证 v13 struct layout。此步仍不接 upstream io-loop、不发送 `Data::SendFiles`、不写 destination，也不启用 product opt-in/UI；双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-receive-block-abi-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-io-loop-receive-interception-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2m Viewer io-loop receive interception lifecycle 已完成自动实现**。Pinned upstream 以单独 tracked patch 新增 `rdn-native-core` feature-gated `InvokeUiSession::native_file_transfer_receive_block`，默认返回 false；`FileResponse::Block` 先询问该 hook，只有返回 false 才保留原 `write_jobs` 路径。`BridgeUi` 仅当 block ID 命中当前 `active_file_download_jobs` 才返回 true：先在锁内复制 `Copy` job 并走 H6.3f2b2k 的 matching/file-range/decode bounds，再释放锁；合法 semantic block 通过 H6.3f2b2l v13 callback 投递，matching malformed block 被消费并 fail closed，unmatched/stale ID 返回 false 保留上游语义。Regression 覆盖 foreign ID passthrough、registered block callback、matching malformed consume-without-callback 与 job removal 后恢复 passthrough。此步仍不发 wire download request、不写 destination、不改 ABI，也不启用 product opt-in/UI；双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-io-loop-receive-interception-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-download-wire-request-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2n Viewer download wire-request lifecycle 已完成自动实现**。`rdn_client_file_transfer_download_start` 仍只接受 active/authenticated/dedicated file session、exact epoch、remote permission、ready sender、exact completed manifest、positive unique transfer ID 与最多 8 个 active job；注册成功后现在直接向现有 session queue 入队一个 canonical 根 `FileAction::Send`：ID 精确等于 transfer ID，path 固定 `/`，file number 固定 0，generic type 且 hidden=false。实现刻意不使用 `Data::SendFiles`，因此不会让 upstream io-loop 创建依赖本地 path 的 `write_jobs`；H6.3f2b2m hook 继续只消费 matching block。job mutex 覆盖 registration 与非阻塞入队，queue 已关闭时先删除刚注册 job 再返回稳定失败；duplicate、manifest mismatch、stale epoch 与其他拒绝路径不会多发命令。Regression 解析真实 protobuf union 验证 exact wire shape、single-dispatch、拒绝路径无副作用及 closed-queue rollback。此步不改 ABI，overwrite digest confirmation 与 callback 到 Swift destination reservation/write/commit 的 adapter 都尚未连接，因此远端 sender 尚不能进入 payload block 阶段；App/Agent opt-in、picker/UI 也继续关闭，产品能力不变，双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-download-wire-request-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-download-digest-confirmation-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2o Viewer download digest-confirmation lifecycle 已完成自动实现**。recursive manifest 完成时，Rust 现在除 totals 外还保留按远端枚举顺序排列的 per-file size/mtime authority；download registration 必须取得 exact manifest files，job 从 file number 0 开始逐项确认。Pinned upstream 通过独立 tracked patch 增加 feature-gated digest hook：foreign/unregistered ID 返回未消费并保留 upstream fallback；matching digest 必须同时满足 non-upload、non-resume、non-identical、transferred size=0、exact next file number 与 exact manifest size/mtime，才通过既有 peer 发送 `FileTransferSendConfirmRequest::OffsetBlk(0)` 并推进一次序号。matching malformed、out-of-order、drift 或 duplicate digest 均被消费但不确认，fail closed 且不能落回 path-based upstream job；receive block 还要求对应 file number 已完成 digest confirmation。实现不改 Viewer ABI，不跨边界传路径或 descriptor。当前 callback 到 Swift destination reservation/write/commit 的 adapter 仍未连接，semantic block 仍被丢弃，App/Agent opt-in、picker/UI 保持关闭；双机传输未验证且不阻塞。本步未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。机器审计与完整验证见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-download-digest-confirmation-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-receive-write-adapter-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2p Viewer receive/write adapter lifecycle 已完成自动实现**。新的 Swift adapter 最多持有 8 个 exact transfer route；`startFileTransferDownload` 必须在 Rust wire start 前同时拿到 manifest-bound request、同 lease 的 destination owner 与 per-transfer terminal sink，Core start 拒绝立即回滚 route。callback queue 先让 adapter 消费 owned semantic block，再允许 observation callback：epoch/ID/file 必须匹配，文件严格按 manifest 顺序；leading/trailing zero-byte files 会以真实 staging+commit materialize，nonempty block 只通过既有 descriptor-relative reservation、128 KiB bounded `pwrite`、exact declared size、mtime、file fsync、`RENAME_EXCL` 与 parent fsync。manifest empty directory 也只从 pinned root 逐组件 no-follow 创建精确 `0700`，final name 必须不存在并 fsync child/parent。只有全部文件与 empty directories 本地完成后才 forward remote completed；premature completed、stale/out-of-order/oversize、metadata/local I/O 均清 staging 并 fail closed，rename 后 parent fsync 失败继续作为 durability-unconfirmed 独立 terminal，不能冒充可重试失败；block failure 通过 lifetime-bound relay 请求 exact Core cancel。disconnect 先关 delivery、清全部 route/staging 再解绑 cancel target。Viewer ABI 保持 v13，产品 App/Agent 没有调用 package start API，session orchestrator、picker/UI 与 Host file opt-in 仍未连接；双机传输未验证且不阻塞。本步只在 UUID 临时目录执行回归，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。下一自动边界为 **host-file-transfer-viewer-session-orchestration-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2q Viewer session orchestration lifecycle 已完成自动实现**。新的 package-scoped owner 固定 one positive connection epoch；由于 upstream `ReadEmptyDirsResponse` 不携带独立 wire request ID，同一 epoch 只允许一个 pending recursive-manifest request，但 manifest-complete download 继续按既有 authority 最多并发 8 个。`beginDownload` 只有在 request ID/transfer ID positive、transfer unique、destination lease exact 且 Core 接受 manifest command 后才消费 destination lifetime；request command reject 不关闭 caller owner。两份 manifest part 经既有 strict authority join 后，owner 原子建立 queued progress、manifest-bound request 与 active destination，再调用 H6.3f2b2p start；Core start reject 关闭已消费 destination。file commit 必须从 0 严格连续，completed 还必须覆盖全部 manifest files；Core completed/cancelled/failed 只有先收到同 transfer receive adapter 的 local completed/cancelled/exact remote-failure proof 才能成为 terminal，否则 exact cancel + receive-route discard 后按 protocol violation 关闭。用户 cancel 先进入 progress cancelling，Core reject 时也 discard exact local route；connection teardown 等待同步 Core command 退出，按 transfer ID 排序 cancel/discard 全部 active route，并关闭 active/pending destination descriptor。新增 exact-transfer progress teardown，避免一个 terminal 清除其他并发下载。Viewer ABI 保持 v13，App/Agent 没有构造此 owner、file opt-in/picker/UI 仍关闭；双机传输未验证且不阻塞。本步只操作 UUID 临时目录，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。下一自动边界为 **host-file-transfer-viewer-product-composition-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2r Viewer product composition lifecycle 已完成自动实现**。由于 v13 的 `fileTransferEnabled=true` 明确定义 dedicated `FILE_TRANSFER` 会话，本步没有复用或改变正在显示画面的 desktop Core；新的 CoreBridge product composition 固定 one positive file epoch，并惰性持有独立 Core factory。只有显式 `start(baseConfiguration:)` 才构造第二个 Core，并把 server/public-key/peer/password/relay 仅同步投影为 clipboard 六方向全关、file enabled 与 exact epoch 的配置；composition 不保留 base configuration 或 credential。独立 Core 报 streaming/file-ready 后才允许 `beginDownload(destinationDirectory:)`，transfer/request ID 与 opaque destination token 单调且不复用，目录仍须通过 pinned current-euid `0700` owner admission；manifest/progress callback 只路由给 exact session owner，认证/连接/非法 control state 以稳定 failure fail closed。显式 teardown 固定先让 session owner cancel/discard route 并关闭 destination，再 disconnect dedicated Core；App 为每次 live Viewer attempt 惰性创建并强持有该 composition，回 Home、启动失败和 App exit 都先停它再断 desktop Core。当前 App 不调用 composition start/beginDownload，且没有 `NSOpenPanel`、按钮、Host file opt-in/root UI，故不会创建第二条网络连接或写真实用户文件，产品文件传输仍关闭；Viewer ABI 保持 v13，双机验收未验证且不阻塞。本步仅用 UUID 临时目录验证 destination admission，未安装/启动 App、修改 Hermes、CI、依赖、数据库或 push。下一自动边界为 **host-file-transfer-viewer-download-picker-action-lifecycle**。

> 更新（2026-08-11）：**H6.3f2b2s Viewer download picker/action lifecycle 已完成自动实现**。live Viewer 的本地控制条只在 desktop Core 到达 streaming 后启用一次“接收文件”动作；fixture 不显示该入口。directory-only、single-selection、alias-closed 的 `NSOpenPanel` 明确要求 current-euid/`0700` 私有目录，selection 立即通过既有 descriptor owner 固定 inode，并在建立任何 file network runtime 前丢弃路径。App 仅保留 server/public-key/peer/relay 与可选 Keychain device ID；用户选定目录后才尝试读取钥匙串密码，缺失/不可用时通过独立 secure sheet 临时取得，随后把 credential 同步交给 composition 并清空临时 projection。composition 的 explicit `requestDownload` 在 idle 时先持有 destination authority 再 start 独立 clipboard-all-off/file-on Core，ready 后才发 recursive manifest；连接前 queued action 可 exact cancel，认证/同步 start 失败及 Home/App teardown 都在 dedicated Core disconnect 前关闭 queued/session destination。清单完成进入 active 后按钮才开放 exact cancel；progress/file-commit/terminal 只按 active transfer ID 更新 UI。由于 upstream empty-directory response 无 request ID，同一 dedicated epoch 仍限制一次 action，完成/取消/失败后不在该 epoch 重用。Viewer ABI 保持 v13，Host App/Agent 仍没有 `fileTransferEnabled` + receive-root 产品 opt-in，因此端到端文件传输尚未开放；双机传输未验证且不阻塞。本步未安装/启动 App、操作真实用户文件、修改 Hermes、CI、根依赖、数据库或 push。下一自动边界为 **host-file-transfer-host-home-receive-root-opt-in-lifecycle**。
...
> 更新（2026-08-11）：**H6.3f2b2t3 Host Home receive-root opt-in lifecycle 已完成自动实现**。Host Home 新增默认关闭的文件接收开关与位置选择；用户只选择 parent，FarPane 以 descriptor-relative `open/openat/mkdirat` 创建或复用固定 `FarPane Receive` child，parent 必须属于当前 euid 且不可 group/world writable，child 必须为当前 euid 的精确 `0700` 真实目录，symlink/unsafe existing child/cancel 均不写偏好。成功时先持久化 canonical root 再置 enabled，关闭时先关 enabled 再删除 root；partial/非 canonical defaults 一律解析为 `.disabled`。策略只可在 Host 关闭、ownership coherent 且无 Viewer 启动中的情况下修改；Home 只显示 basename。后台 Agent 经 schema v5 bootstrap、legacy Host 经直接配置共同投影同一 policy 到 Host ABI v17，Core 启动仍执行 authoritative descriptor admission。此步 filesystem regression 只使用 UUID 临时目录，未启动 GUI、操作用户目录、安装、修改 Hermes、CI、依赖、数据库或 push；双机验收未验证且不阻塞。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-host-home-receive-root-opt-in-lifecycle.md`；下一自动边界为 **host-file-transfer-product-development-completion-audit**。

> 审计（2026-08-11）：**H6.3g product development completion audit 已完成，H6.3 开发仍未完成**。当前 37 份 H6.3 机器审计全部可重跑通过；独立默认关闭权限、Host Home receive-root opt-in、Host descriptor-owned receive/send、Viewer 独立 file session/download picker、manifest、bounded receive、durable no-replace commit、progress/cancel/terminal 与安全 root/path/payload 门禁均已接通。唯一剩余产品代码缺口是 FarPane Viewer 发送：当前 `ViewerFileTransferDirection` 只有 `download`，控制条只有“接收文件”，没有文件/目录选择、本地 descriptor-owned manifest authority、upload request 或 wire/ABI dispatch，因此“全部使用 FarPane”尚不能从控制端发送文件到 Host。overwrite/replace 与多文件 resume 当前保持明确 fail closed；完整文件管理器是 V1 非目标，不作为缺口。Mac mini 安装版单机 smoke、双机双向传输、性能与互操作继续按 §3.4 记为非阻塞未验证。本步只审计源码和既有机器证据，不启动 GUI、访问真实用户文件、安装、修改 Hermes、CI、依赖、数据库或 push。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-product-development-completion-audit.md`；下一自动边界为 **host-file-transfer-viewer-upload-selection-manifest-contract**，先建立默认未接产品、无需 ABI 的本地选择与安全 manifest authority。

> 更新（2026-08-11）：**H6.3h Viewer upload selection/manifest contract 已完成自动实现**。新增默认未接产品的多文件/目录 `NSOpenPanel`，关闭 alias 解析；选择 URL 必须立即进入 CoreBridge descriptor owner，之后只发布 exact-session opaque lease、NFC 规范化 manifest 与 path-free upload request。top-level selection 用 `open(O_NOFOLLOW)` 固定，目录递归只经 pinned descriptor 的 `fdopendir/fstatat/openat`；拒绝 symlink、跨 mount、特殊文件、不安全 owner/mode、控制字符、private staging 与 case-fold collision，目录内隐藏项默认忽略，并限制 64 个 top-level selections、1,024 manifest entries、64 层、1 MiB metadata 与 checked total bytes。borrow 前后重验 device/inode/owner/mode/link/size/mtime/ctime；原选择路径被替换成外部 symlink 仍读取原 inode，同 size/mtime 的内容漂移也 fail closed，exact epoch teardown 确定性关闭 descriptor。Viewer ABI 保持 v13，App/ViewerUI 尚无上传动作、wire dispatch 或产品构造，能力继续未开放。本步只操作 UUID 临时目录，未启动 GUI、访问真实用户目录、安装、修改 Hermes、CI、依赖、数据库或 push；双机上传未验证且不阻塞。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-upload-selection-manifest-contract.md`；下一自动边界为 **host-file-transfer-viewer-upload-wire-abi-ownership-audit**。

> 审计（2026-08-11）：**H6.3i Viewer upload wire/ABI ownership audit 已完成，未改 ABI**。Pinned upstream 的 `Data::SendFiles` 会把本地字符串路径包装成 `DataSource::FilePath`，随后由 Rust `TransferJob::new_read` 自行 reopen/read，因此不能复用来消费 H6.3h 已固定在 Swift descriptor owner 中的来源。可复用的是 canonical wire：Rust 继续发送 `FileAction::Receive`、逐文件 digest/confirm、至多 128 KiB `FileTransferBlock`、Done/Cancel；Native Host 已在同一 wire 上接收。下一实现冻结为 **Viewer ABI v14** 的最小 semantic seam：`RDNFileTransferUploadStart` 只携带 epoch、transfer ID、opaque source lease 与 bounded normalized manifest；`RDNFileTransferUploadReadRequest` 只携带 exact file number and offset。Rust owns transfer state、manifest projection、digest/confirm、block sequencing、wire compression、progress/terminal；Swift owns source lifetime，并通过 synchronous read callback 把数据直接填入 Rust caller-owned 128 KiB buffer。路径和 descriptor must not cross the ABI，双方 must not retain the callback buffer。Rust 只接受 exact `OffsetBlk(0)`；non-zero offset、resume、overwrite、越界/短读、duplicate callback、stale epoch/lease must fail closed，existing-target 回应只允许 `Skip(true)` no-replace。manifest 中 empty directories 不伪装成 file entry，而由 Rust 以 canonical create action（`FileAction::Create`）按父路径顺序投影；任一失败终止同一 upload lifecycle。multi-file resume 继续关闭。本审计只冻结 ownership/ABI 方向，当前 header 仍为 v13，wire/upload product action 均未实现，双机上传未验证且不阻塞。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-upload-wire-abi-ownership-audit.md`；下一自动边界为 **host-file-transfer-viewer-upload-semantic-read-abi-contract**。

> 更新（2026-08-11）：**H6.3j Viewer upload semantic-read ABI contract 已完成自动实现**。Viewer ABI 从 v13 升至 v14，新增 path-free `RDNFileTransferUploadStart` 与同步 `RDNFileTransferUploadReadRequest` callback：start 只借用 bounded normalized manifest，并以 exact epoch、positive transfer ID、opaque source token 注册最多 8 个 Rust-owned semantic job；Swift 在调用 Core 前先绑定同一 request 与 descriptor owner，Core 拒绝即回滚 route。Rust 为每次读取分配最大 128 KiB caller-owned buffer，只请求 manifest 内 exact file number、offset/length 范围，并要求 callback 返回精确 `bytes_written`；Swift 只在 exact epoch/transfer/token route 下通过 pinned descriptor `pread`，borrow 前后继续复核 inode/owner/mode/link/size/mtime/ctime。短读、越界、stale route、duplicate job、cross-direction transfer ID、权限/session drift 与 callback 期间 teardown 均 fail closed，拒绝后的 buffer 会清零。cancel、terminal、disconnect 和 worker exit 会同时清除 Rust semantic job 与 Swift source route。此步刻意只建立来源 ownership/read seam，不发送 `FileAction::Receive/Create`、digest、block 或 done，也不增加 App/ViewerUI 上传动作，因此产品上传仍未开放；双机上传继续记为未验证且不阻塞开发。本步测试只读取 UUID 临时目录中的 fixture，未启动 GUI、访问用户文件、安装、修改 Hermes、CI、依赖、数据库或 push。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-upload-semantic-read-abi-contract.md`；下一自动边界为 **host-file-transfer-viewer-upload-wire-job-lifecycle**。

> 更新（2026-08-11）：**H6.3k Viewer upload wire-job lifecycle 已完成自动实现**。Rust 现在把 ABI v14 semantic upload job 投影为 canonical `FileAction::Create`（空目录祖先 parent-first）、`FileAction::Receive`（Host receive-root）、逐文件 digest/confirm、最大 128 KiB block、Done/Cancel 生命周期；io-loop 以最多 8 个 job 的轮转 poll 驱动，每次 tick 最多读取并发送一个 block，活跃/等待阶段分别使用 1 ms/100 ms 调度，任何阶段超过 30 秒即 unavailable fail closed。只接受 exact `OffsetBlk(0)` 或 no-replace `Skip(true)`；nonzero offset、`Skip(false)`、错序/错 ID、短读、local I/O 与协议漂移都会清除 job、发稳定 terminal，并按需发送 Cancel。Host write owner 同时允许空 base path 精确表示已固定 receive-root，并在 digest 确认时 durable materialize 零字节文件，避免无 block 文件无法推进。tracked io-loop patch 已进入 pinned bootstrap apply/reverse-check；Swift 仍独占 source descriptor，路径与 descriptor 不跨 ABI。产品上传入口仍未开放，existing-target replace 与 multi-file resume 继续关闭；双机上传继续记为未验证且不阻塞开发。本步只读写测试 UUID 临时目录，未启动 GUI、访问真实用户文件、安装、修改 Hermes、CI、依赖、数据库或 push。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-upload-wire-job-lifecycle.md`；下一自动边界为 **host-file-transfer-viewer-upload-product-action-lifecycle**。

> 更新（2026-08-11）：**H6.3l Viewer upload product action lifecycle 已完成自动实现**。live Viewer 控制条现在并列提供“接收文件”和“发送文件”；发送动作使用 file/directory、多选、alias-closed 的显式 `NSOpenPanel`，selection 在发起网络连接前立即进入既有 descriptor-owned upload source，后续只向 ABI v14 提交 normalized manifest、exact epoch/transfer ID 与 opaque source lease，路径和 descriptor 不跨 ABI。App 沿用 download 的惰性凭据策略：只保留非秘密连接上下文和可选 Keychain device ID，用户选定来源后才读取或安全提示访问密码，同步投影给独立 clipboard-all-off/file-only Core 后清空临时密码。composition 对 download/upload 共用单调 transfer ID 且同一 dedicated epoch 只消费一个显式动作；upload owner 对 totals、单调 progress、terminal、cancel 与 protocol drift 执行 exact-session fail-closed，queued cancel、认证失败、Home/App teardown 都在 Core disconnect 前关闭 source route 与 pinned descriptors。UI 只允许当前方向发起取消，另一方向在 active/one-shot epoch 内禁用；完成、取消、失败均按 upload direction 呈现。Host file receive 仍需被控端显式 opt-in，overwrite/replace 与 multi-file resume 保持关闭。代码、focused regression、全量 Swift/脚本测试、静态审计和 fresh App build 通过后，H6.3 产品开发代码无剩余缺口；Mac mini 安装版单机 smoke 是下一自动边界，双机上传仍未验证且不阻塞开发完成。本步未启动 GUI、访问真实用户文件、安装、修改 Hermes、CI、根依赖、数据库或 push。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-viewer-upload-product-action-lifecycle.md`；下一自动边界为 **host-file-transfer-installed-single-mac-smoke**。

> 更新（2026-08-11）：**H6.3m build-bundle single-Mac preflight 已完成，installed-new-build smoke 保持未验证且不阻塞**。旧 golden preflight 直接 reverse-check 最底层 Rust patch，在 H3–H6 后续层已经叠加时会误报；新的只读验证器改为确认 pinned RustDesk commit、当前 Viewer upload 与 native read 顶层、hbb_common 两个顶层及三份 canonical bridge source，并由回归测试证明执行前后两个 checkout 状态不变。最新 `Build/FarPane.app` build `202608111329` 通过 stable codesign、release/App/Core UUID 对账、sanitized diagnostic、真实 ScreenCaptureKit → hardware H.264 与 no-window `--help` smoke；ScriptTests 180/180。当前运行中的 `/Applications/FarPane.app` 是 build `20260808131034`，其 Core 不含 upload ABI，因此本步没有停止、覆盖或并行启动它，`installedSingleMacSmokeComplete=false`；双机传输同样未验证但不阻塞产品开发完成。未安装/启动 GUI、修改 Hermes、CI、依赖、数据库或 push。证据见 `Evidence/HostMode/2026-08-11/h6-host-file-transfer-build-bundle-single-mac-preflight.md`；下一自动开发边界进入 **H6.4 selectDisplay / revisioned display mapping ownership audit**。

> 审计（2026-08-11）：**H6.4a multi-display selection ownership audit 已完成，下一步需要 Viewer ABI 检查点**。Pinned RustDesk 已有登录/动态 `PeerInfo.displays` inventory、canonical `Session::switch_display`、Host old/new monitor service subscription 切换、同 video channel 的 `SwitchDisplay` echo→frame ordering，以及 macOS connection-scoped input-mapping generation；FarPane Viewer ABI v14 却把 `set_peer_info`/`set_displays`/`set_current_display` 全部丢弃，`switch_display` 只更新尺寸，帧只有裸 display index，C/Swift ABI、UI 与 pending-input 生命周期均没有 selection seam。机器审计据此冻结 Viewer ABI v15：catalog identity 为 `connectionEpoch + catalogRevision + displayIndex`，initial/dynamic/remote-follow/echo ingress 分权，positive unique command ID + exact revision 只表示 admission，matching echo 或 typed already-selected 才产生唯一 terminal event；catalog change/disconnect/identity drift fail closed，catalog 变化推进 revision，帧绑定同一 tuple，pending 期间 release/pause input。Host 还须在改变 service subscription 前拒绝 negative/out-of-range/offline/non-positive geometry。Viewer catalog revision 不得复用 Host media `displayRevision` 或 Host input mapping generation。Host ABI 保持 v17，protobuf wire 与 Hermes 均无需修改；审计 8/8 evidence、8/8 gaps、8/8 anchors，focused 1/1、全量 ScriptTests 181/181、Python/diff checks 与 fresh arm64 release build 均通过，当前不宣称多显示器产品完成。证据见 `Evidence/HostMode/2026-08-11/h6-multi-display-selection-ownership-audit.md`；下一自动边界为 **viewer-display-catalog-abi-contract**，双机多显示器/scale/rotation/hot-plug/input 验收继续未验证且不阻塞自动开发。

> 更新（2026-08-11）：**H6.4b Viewer display catalog ABI contract 已完成自动实现**。Viewer ABI 从 v14 升至 v15，Rust 现在从 initial/dynamic inventory、remote-follow 与 `SwitchDisplay` echo 归一化 connection-scoped catalog；目录最多 64 项、名称最多 512 UTF-8 bytes，非法 geometry、control character、非 finite/非正 scale 或超限 inventory 整体投影 typed unavailable。首次状态 revision=1，semantic duplicate 不推进，inventory/status change 推进，selection-only change 保持 revision；每帧绑定 exact `connectionEpoch + catalogRevision + displayIndex`，Rust 只发布当前可用选择的帧，Swift 在复制 callback-scoped packet 前及 queued delivery 前双重丢弃 stale/mismatched tuple。dedicated file session 不发布目录，disconnect/worker exit 清除 authority；display name 仅用于呈现，不进入 identity/diagnostics。Host ABI 仍为 v17，protobuf/Hermes 未改。本步没有 `selectDisplay` command、terminal event、产品 selector/input quiescence 或 Host range validation，故不宣称 H6.4 产品完成；双机多显示器/scale/rotation/hot-plug/input 验收继续未验证且不阻塞。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-display-catalog-abi-contract.md`；下一自动边界为 **viewer-select-display-command-lifecycle**。

> 更新（2026-08-11）：**H6.4c Viewer display-selection command/terminal lifecycle 已完成自动实现**。Viewer ABI 从 v15 升至 v16；请求只接受 active desktop connection 的 exact epoch、当前 available catalog revision、可选 display index 与严格递增的正 command ID，每连接最多一个 pending command。当前 index 不发送 wire 并产生 typed `alreadySelected`；其他选择复用 canonical upstream `SwitchDisplay`，且只有未变化 revision 下的 matching echo 才产生 `selected`。catalog change、disconnect 或 remote selection drift 各自以 typed failure 唯一终结已 admission 的命令；函数返回值仍只表示 admission。C header/shim/Rust authority/Swift strict projection 与 build symbol gate 已同步，Host ABI 保持 v17，protobuf/Hermes 未改。本步仍不包含产品 selector、pending 期间 input release/pause 或 Host negative/out-of-range/offline/non-positive geometry validation，故不宣称 H6.4 产品完成；双机现场验收继续未验证且不阻塞。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-display-selection-command-lifecycle.md`；下一自动边界为 **host-display-switch-validation-lifecycle**。

> 更新（2026-08-11）：**H6.4d Host display-switch validation lifecycle 已完成自动实现**。Host monitor 切换入口现在只接受自身 live display-service inventory 中的目标，并在任何 service subscription/input mapping mutation 前拒绝 negative/不存在 index、offline、非正 width/height、非 finite/非正 scale 与坐标边界溢出。拒绝时不静默悬挂 Viewer pending command，而是通过既有 canonical `SwitchDisplay` response 重申当前 display，使 Viewer 以 typed `remoteSelectionDrift` 唯一终结；合法目标继续走原 upstream service 切换与 input-mapping generation advance。该实现以独立 canonical patch 接在当前 RustDesk patch stack 顶层，bootstrap/verifier 均验证其可重放；Viewer ABI 保持 v16、Host ABI 保持 v17，protobuf/Hermes 未改。本步仍不含产品 selector 与 pending 期间 input release/pause，故不宣称 H6.4 产品完成；双机现场验收继续未验证且不阻塞。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-display-switch-validation-lifecycle.md`；下一自动边界为 **viewer-display-selection-input-quiescence-lifecycle**。

> 更新（2026-08-11）：**H6.4e Viewer display-selection input quiescence lifecycle 已完成自动实现**。新增 connection/catalog/command-scoped 的 Viewer input owner：只有当前 available catalog 的 online display 可发起命令，先同步释放 held pointer、普通键盘/输入法与独占键盘，再调用既有 Viewer ABI v16 `selectDisplay`；Core 同步拒绝会回滚本次暂停，admission 后则保持 fail-closed。stale terminal、错误 identity、catalog 尚未投影到目标 display 或 typed failure 均不会恢复输入；只有 exact terminal success 与当前 `connectionEpoch + catalogRevision + selected displayIndex` 同时匹配才恢复。普通 Viewer event path 全部受同一 gate 约束，待发送 mouse move 在暂停时直接丢弃，独占键盘保留用户 intent 但窗口回焦也不能绕过 gate；Core replacement 继承暂停态，teardown 不以旧连接身份恢复输入。App 已接入 catalog/selection callbacks 并预留内部 selection action，但本步刻意不增加可见显示器 selector，因此 H6.4 产品仍未完成。Viewer ABI 保持 v16、Host ABI 保持 v17，protobuf/Hermes 未改；双机多显示器/scale/rotation/hot-plug/input 验收继续未验证且不阻塞开发。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-display-selection-input-quiescence-lifecycle.md`；下一自动边界为 **viewer-display-selector-product-lifecycle**。

> 更新（2026-08-11）：**H6.4f Viewer display selector product lifecycle 已完成自动实现**。live Viewer 控制条新增原生显示器选择器，目录只由当前 Core generation/attempt 的 normalized catalog 投影；菜单项以 canonical `displayIndex` tag 路由动作，远端 name 仅截断后与分辨率共同呈现，不进入 identity 或诊断。offline 项可见但不可选；catalog unavailable、尚未取得 control authority、pending、typed failure 与 Core replacement 继承的 fail-closed pause 都有独立状态。选择动作先进入 H6.4e input owner；pending 时 selector 禁用，admission rejection/目录变化/连接关闭/remote drift 均显示稳定脱敏提示并允许在 authority 恢复后显式重试，exact success 才恢复当前项与远程输入。fixture 不显示 selector，live session teardown/replacement 不复用旧目录 authority。Viewer ABI 保持 v16、Host ABI 保持 v17，protobuf/Hermes 未改；H6.4 ownership audit 现有 13/13 evidence 且无代码 gap，但本步不把尚未执行的双机多显示器/scale/rotation/hot-plug/picture/input 验收伪装为完成。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-display-selector-product-lifecycle.md`；下一自动边界为 **multi-display-product-development-completion-audit**。

> 更新（2026-08-11）：**H6.4g multi-display product development completion audit 已完成**。新的机器审计把 §8.4/§12.1/阶段 8 的 `selectDisplay`、revisioned display mapping、Host 切换前验证、Viewer 输入 fail-closed 与产品 selector 逐项映射到 H6.4a–f 的实现、测试和证据；它组合既有 ownership audit 的 13/13 evidence/no gaps，并确认 completion matrix 11/11 evidence、13/13 source anchors、0 development gap。按 §3.4，H6.4 的代码开发现已完成；installed current-build GUI smoke 与双机 display inventory/selection、picture recovery、scale/rotation/hot-plug、切换后 pointer/keyboard mapping、跨机性能仍明确为未验证，不伪装为通过，也不重新打开代码缺口。Viewer ABI 保持 v16、Host ABI 保持 v17，protobuf/Hermes 未改；未安装/启动 GUI、修改 Hermes、CI、依赖、数据库或 push。机器证据见 `Evidence/HostMode/2026-08-11/h6-multi-display-product-development-completion-audit.md`；下一自动开发边界为 **host-audio-product-ownership-audit**。

> 审计（2026-08-11）：**H6.1a Host audio product ownership audit 已完成，产品实现尚未开始**。Release Core 未启用上游 `screencapturekit` feature，因此 pinned RustDesk 的 Host audio service 现有主路是 cpal native system-default microphone，并继续拥有 Opus 编码、`AudioFormat`/`AudioFrame` wire 与 connection-scoped subscription；Viewer `Remote` audio thread/`AudioHandler` 已拥有 Opus 解码、buffer/resample/rechannel 与 native output，音频 payload 无需穿过 Swift。FarPane 当前 Host start 固定并 readback `enable-audio=N`，native Viewer 也固定 `disable_audio=true`，两端均未产品启用。机器审计确认 11/11 ownership evidence、8/8 gaps、14/14 source anchors：缺少 Host/Viewer default-off audio policy ABI、microphone TCC/usage description、bootstrap/Home opt-in、Viewer remote permission projection、local-policy-aware approval request 与 virtual input selector。后续合同冻结为 Host ABI v18 `enableAudio` 和 Viewer ABI v17 `receiveAudio`；麦克风为默认主路，system audio 只允许用户显式选择第三方虚拟 input device，不启用自动 ScreenCaptureKit loopback。RustDesk wire、Hermes、根依赖与数据库均无需修改；双机音频、撤权、延迟/CPU/互操作验收仍未验证且不阻塞自动开发。证据见 `Evidence/HostMode/2026-08-11/h6-host-audio-product-ownership-audit.md`；下一自动边界为 **host-audio-explicit-policy-abi-contract**。

> 更新（2026-08-11）：**H6.1b Host audio explicit-policy ABI seam 已完成自动实现**。Host ABI 从 v17 升至 v18，`RdnHostCreateOptions` 新增 immutable `enable_audio`；Swift `HostServerConfiguration.audioEnabled` 默认 `false`，现有 App/Agent 调用方未显式开启，因此产品行为保持关闭。Rust 在读取 identity 前把 audio、clipboard、file 三项独立策略写入 pinned config，并在 start preflight 对 `enable-audio=Y/N` 做 exact readback；本地 approval 只有在 Host audio policy 开启且远端未禁用音频时才请求兼容 capability `hearSystemAudio`，实际 subscription 继续消费同一 persisted option。Release Core 仍不启用 `screencapturekit`，音频 payload、Opus 与 wire 留在 RustDesk，Viewer ABI 保持 v16 且仍默认禁用接收音频。本步没有产品 opt-in、麦克风 TCC/usage description、Viewer audio policy、虚拟 input selector、安装或双机验收，也未修改 Hermes、protobuf、CI、根依赖、数据库或 push；这些未验证项不阻塞本 ABI seam。证据见 `Evidence/HostMode/2026-08-11/h6-host-audio-explicit-policy-abi-contract.md`；下一自动边界为 **host-audio-bootstrap-microphone-opt-in-contract**。

> 更新（2026-08-11）：**H6.1c Host audio bootstrap and microphone opt-in contract 已完成自动实现**。Host Agent immutable bootstrap 从 schema v5 升至 v6，新增严格 Boolean `audio.enabled`；schema v1–v5 一律迁移为关闭，额外键、数值伪装 Boolean 与 future schema fail closed。Host Home 新增“远程音频（默认关闭）”显式开关，只有前台主 App 的用户动作可调用 `AVCaptureDevice.requestAccess(for: .audio)`；未决定状态只允许一个在途请求，完成后重新读取 TCC 权威状态，不能用 callback Boolean 越权。开启偏好只在麦克风已授权且 Host 关闭、没有 Viewer 启动中、没有授权请求在途时写入；权限撤销会把有效策略投影为关闭。后台 HostAgent 绝不触发弹窗，只把已验证 bootstrap policy 与无提示 TCC 状态相交后传给 Host ABI v18；legacy Host 使用同一有效策略。`NSMicrophoneUsageDescription` 已加入现有 `RustDeskNative` executable / `io.rustdesknative.viewer` bundle，避免创建第二套 TCC identity。Viewer 仍固定不接收音频，虚拟输入设备选择与安装/双机验收仍未实现且不阻塞本步；未触发 TCC、启动 GUI、安装、修改 Hermes、CI、根依赖、数据库或 push。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-audio-bootstrap-microphone-opt-in-contract.md`；下一自动边界为 **viewer-audio-explicit-policy-abi-contract**。

> 更新（2026-08-11）：**H6.1d Viewer audio explicit-policy ABI contract 已完成自动实现**。Viewer ABI 从 v16 升至 v17，`RDNConnectionConfig` 与 Swift `CoreConnectionConfig` 新增 immutable `receive_audio` / `receiveAudio`，Swift 默认 `false`，现有 App 与 file-transfer product composition 均未显式开启，因此产品行为继续关闭。Rust 在创建连接 epoch、启动 worker 或发送 login 前读取该 policy，并把 `false` 精确投影为 `disable_audio=true`、`true` 投影为 `false`；既有 RustDesk `OptionMessage.disable_audio`、`AudioFormat`/`AudioFrame`、Opus decode 与 native playback 继续作为唯一数据面，Swift 不接触音频 payload。dedicated file-transfer session 与任一 desktop clipboard/audio capability 互斥，不能借 file session 隐式开启播放。Viewer 远端音频 permission 呈现、产品 opt-in、虚拟输入选择及安装/双机验收仍未实现且不阻塞本 ABI seam；Host ABI 保持 v18，未修改 Hermes、protobuf wire、CI、根依赖、数据库或 push。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-audio-explicit-policy-abi-contract.md`；下一自动边界为 **viewer-audio-product-opt-in-permission-lifecycle**。

> 更新（2026-08-11）：**H6.1e Viewer audio product opt-in and permission lifecycle 已完成自动实现**。Viewer ABI 从 v17 升至 v18，新增 connection-scoped typed remote-permission event；Rust 在认证时和 `Permission::Audio` 变化时投影 authoritative 状态，Swift 只接受首个正 `connectionEpoch` 并拒绝 replacement/stale event。Home 的“本次连接接收远端音频（默认关闭，断开后重置）”是非持久化、单次 opt-in；普通连接和自动恢复都固定同一 immutable `receiveAudio`，file-transfer 与环境默认路径仍关闭。Rust 播放闸门现在是 local opt-in 与 remote permission 的交集，`AudioFormat`/`AudioFrame` 均在入队前检查；远端拒绝或撤销会先关闸并向 audio thread 发送 `Reset`，释放现有 native playback state，产品控制条分别呈现等待、接收、未授权、撤销和停止。Host ABI 保持 v18，音频 payload、Opus 和 RustDesk wire 未进入 Swift；虚拟输入设备选择、安装及双机音频/撤权/延迟验收仍未实现且不阻塞本开发边界。未启动 GUI、修改 Hermes、CI、根依赖、数据库或 push。机器证据见 `Evidence/HostMode/2026-08-11/h6-viewer-audio-product-opt-in-permission-lifecycle.md`；下一自动边界为 **virtual-audio-input-selection**。

> 更新（2026-08-11）：**H6.1f Host virtual audio input selection ABI contract 已完成自动实现**。Host ABI 从 v18 升至 v19，`RdnHostCreateOptions` 与 Swift `HostServerConfiguration` 新增 create 期间复制的 optional exact input-device name；null/empty 仍唯一表示 system-default microphone，显式名称只在 audio policy 开启时允许，并拒绝空白包围、控制字符和超过 512 UTF-8 bytes 的值。Rust 在读取 identity 前把同一选择写入隔离 config 的 `audio-input`，start preflight 要求 exact readback；pinned audio service 在 native Host feature 下只接受唯一同名设备，找不到或同名歧义都 fail closed，不再悄然回退默认麦克风。现有 App/Agent/bootstrap 均未投影选择，因此产品仍使用默认麦克风，也没有可见 selector；音频 payload、Opus、RustDesk wire 与 Viewer ABI v18 不变。未枚举真实设备、触发 TCC、启动 GUI、安装、修改 Hermes、CI、根依赖、数据库或 push；双机音频与设备消失现场验收继续未验证且不阻塞开发。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-virtual-audio-input-selection-abi-contract.md`；下一自动边界为 **host-audio-bootstrap-virtual-input-selection-contract**。

> 更新（2026-08-11）：**H6.1g Host audio bootstrap and virtual-input selection contract 已完成自动实现**。Host Agent immutable bootstrap 从 schema v6 升至 v7，`audio` 新增 optional exact `inputDeviceName`；schema v6 的 enabled policy 迁移为默认麦克风，v7 则要求关闭与 null 精确成对，显式名称继续复用 512 UTF-8 bytes、无首尾空白和控制字符门禁。Home 通过 CoreAudio 只枚举真实拥有 input channel 的设备，以 exact display name 投影系统默认麦克风、唯一设备与手动刷新；同名歧义、非法名称、读取失败或已选设备消失时显式路径 fail closed，绝不回退默认麦克风。选择只能复用 Host 关闭、无 Viewer 启动中且无 TCC 请求在途的既有策略门禁；默认仍不启用音频，也不自动安装 BlackHole 等第三方设备。有效选择同时进入 canonical bootstrap revision、后台 HostAgent 与 legacy Host 的 Host ABI v19 create 配置。未触发 TCC、安装/启动 GUI、修改 RustDesk wire、Hermes、CI、根依赖、数据库或 push；单机真实设备菜单与双机音频/撤权/延迟仍未验证且不阻塞当前开发口径。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-audio-bootstrap-virtual-input-selection-contract.md`；下一自动边界为 **host-audio-product-development-completion-audit**。

> 更新（2026-08-11）：**H6.1h Host audio product development completion audit 已完成**。新的 completion matrix 组合 H6.1a ownership 与 H6.1b–g 六份阶段审计，逐项证明 Host default-off policy/approval/revoke、主 App 麦克风 TCC、schema v7/bootstrap/Home opt-in、Viewer connection-scoped receive opt-in/remote permission/playback reset、CoreAudio exact input selector 和设备漂移 fail-closed 都已进入产品链；Opus 编解码、`AudioFormat`/`AudioFrame` 与 native capture/playback 继续由 pinned RustDesk 数据面唯一拥有。ownership audit 当前 16/16 evidence、23/23 source anchors、0 development gap；completion audit 为 12/12 evidence、16/16 source anchors、0 remaining development gap。按 §3.4，H6.1 代码开发现已完成；installed current-build 单机设备枚举/TCC、默认麦克风与虚拟输入实际打开、双机声音/拒绝/撤权/设备消失、延迟/CPU/互操作仍明确为未验证，不伪装为通过，也不重新打开代码缺口。默认音频继续关闭，不自动安装 BlackHole；Host ABI 保持 v19、Viewer ABI 保持 v18，RustDesk wire、Hermes、CI、根依赖和数据库未改。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-audio-product-development-completion-audit.md`；下一自动边界为 **host-mode-development-completion-audit**。

> 更新（2026-08-15）：**H6.1i native system-audio loopback product upgrade 已实现，双机现场验收待完成**。Release Rust Core 现在在 `arm64` 与 `x86_64` 原生构建统一启用 pinned RustDesk 的 `screencapturekit` audio backend；schema v7/Host ABI v19 不变，`audio.inputDeviceName = null` 从旧的 system-default microphone 语义升级为 ScreenCaptureKit 原生系统音频，显式 exact CoreAudio input name 继续表示麦克风、BlackHole 或其他输入设备。Home 默认项改为“系统音频（原生）”；系统音频只消费既有屏幕录制授权，只有显式输入设备才与麦克风 TCC 相交并由前台 App 请求授权，后台 HostAgent 不弹窗。Viewer 仍为每连接、默认关闭的明确接收开关，Opus、`AudioFormat`/`AudioFrame`、远端授权与撤权继续留在 RustDesk 数据面；新增 `NSAudioCaptureUsageDescription`，未改 wire、Hermes、ABI 或 bootstrap schema。自动测试与双架构构建结果以本次交付证据为准；Mini→MBP、MBP→Mini 的真实声音、音画同步、重连、锁屏/睡眠及 CPU/延迟仍必须分别现场验收，不能由编译通过替代。

> 更新（2026-08-11）：**H6.2l clipboard product development completion audit 已完成**。新的 completion matrix 精确组合 H6.2a–k5 的 22 份机器审计，并从当前 Host/Viewer ABI、Rust directional policy 与 bounded envelope、schema v7 bootstrap、Home 六方向开关、Agent/legacy projection、Viewer session-scoped AppKit owner、动态退避和 teardown 中重新取得 14 个源码锚点；small text、RTF/HTML、RGBA/PNG/SVG 三类产品链全部闭合，read/write 与 session revoke 继续独立，所有 Core 默认值与 Host Home 偏好继续 default off。审计为 12/12 evidence、14/14 source anchors、0 remaining development gap，因此按 §3.4，H6.2 代码开发现已完成；installed current-build 单机 pasteboard smoke、双机小文本/富文本/图片 round trip、方向撤权/重连、跨机性能与互操作仍明确为未验证，不伪装为通过，也不重新打开代码缺口。Host ABI 保持 v19、Viewer ABI 保持 v18，RustDesk wire、Hermes、CI、根依赖和数据库未改，未读取真实剪贴板、安装或启动 GUI、push 或部署。机器证据见 `Evidence/HostMode/2026-08-11/h6-host-clipboard-product-development-completion-audit.md`；下一自动边界为 **host-mode-development-completion-audit**。

> 完成审计（2026-08-11）：**Host Mode H0–H6 开发工作已按 §3.4 完成，状态为 `development-complete-acceptance-pending`**。整体机器审计重新运行 18 份阶段/产品 audit，并用当前仓库取得 13/13 completion evidence 与 43/43 source anchors：H0 许可证、pinned source、patch/config 边界；H1 Host Control、Golden media 与双 codec；H2 telemetry/signpost/dirty rect/adaptive cadence、两级 bounded queue、drop ledger、硬编探测和真实场景采集/验证工具；H3 永久密码、审批、失败冷却、session capability/revoke 与 semantic input gate；H4 pre-AppKit Agent、SMAppService、签名 XPC、snapshot-first、单写者/config-root/persistence readback；H5 sleep/network/display/session recovery 和现场证据编排；H6 音频、剪贴板、双向文件传输、多显示器四项独立 completion audit 均无剩余开发缺口。该结论只关闭代码开发目标：installed current-build 单机 GUI/HostAgent smoke、双机画面/输入/剪贴板/文件/多显示器、Direct/Relay 与双 active session、真实 1080p/4K 性能窗口、Apple Silicon/Intel 30 分钟稳定性、sleep/network/display/session 现场切换、Developer ID notarization/stapling/quarantine/firewall 与发布互操作全部继续明确为未验证，不能宣称发布验收通过。Host ABI v19、Viewer ABI v18、Host Media ABI v1；Hermes、CI、根依赖和数据库未改，未安装/启动 GUI、push 或部署。机器证据见 `Evidence/HostMode/2026-08-11/host-mode-development-completion-audit.md`；当前没有下一自动代码边界，后续只在设备/发布条件具备时选择性补现场验收。
...
- H6.1 音频：ScreenCaptureKit 原生系统音频为默认来源；麦克风或第三方虚拟设备（如 BlackHole）保留为显式可选输入；
