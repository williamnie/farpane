...
/// Real-time HEVC VideoToolbox encoder for the FarPane-to-FarPane Host path.
/// Like the H.264 adapter, it reports hardware state only after a successful
/// compressed callback and emits bounded 4-byte AVCC access units.
public final class HostHEVCEncoder: @unchecked Sendable {
  ...
  private let configuration: HostHEVCEncoderConfiguration
  private let lock = NSLock()
  private let stateQueue = DispatchQueue(label: "io.farpane.host-hevc-encoder-state")
  ...
  public init(
    ...
    guard status == noErr, let created else { throw HostHEVCEncoderError.create(status) }
    session = created
    do {
      ...
      guard prepared == noErr else { throw HostHEVCEncoderError.prepare(prepared) }
    } catch {
  ...
  public func encode(
    ...
    guard let session = lock.withLock({ self.session }) else {
      throw HostHEVCEncoderError.encode(kVTInvalidSessionErr)
    }
    ...
    // before this function returns, so reading infoFlagsOut and releasing the
    // same context here would double-release it. The callback's infoFlags is
    // the single authority for accepted-frame completion and drop handling.
  }
  ...
  private func set(_ key: CFString, value: CFTypeRef) throws {
    guard let session else { throw HostHEVCEncoderError.property(key, kVTInvalidSessionErr) }
    let status = VTSessionSetProperty(session, key: key, value: value)
    guard status == noErr else { throw HostHEVCEncoderError.property(key, status) }
  }
  ...
  private func handle(
    ...
    guard CMSampleBufferDataIsReady(sampleBuffer),
      ...
      throw HostHEVCEncoderError.malformedSample
    }
    let attachments =
    ...
    if isKeyframe, let format = sampleBuffer.formatDescription {
      let parameterSets = try Self.hevcParameterSets(from: format)
      for parameterSet in parameterSets {
        var length = UInt32(parameterSet.count).bigEndian
        withUnsafeBytes(of: &length) { data.append(contentsOf: $0) }
        data.append(parameterSet)
      }
      hasParameterSets = parameterSets.count >= 3
    }
    let payloadLength = CMBlockBufferGetDataLength(blockBuffer)
    guard payloadLength > 0 else { throw HostHEVCEncoderError.malformedSample }
    ...
    guard copied == kCMBlockBufferNoErr else { throw HostHEVCEncoderError.malformedSample }
    data.append(payload)
    let presentationTimeUS = UInt64(
      max(
        0,
  ...
  private static func hevcParameterSets(
    ...
    guard countStatus == noErr, count >= 3, headerLength == 4 else {
      throw HostHEVCEncoderError.malformedSample
    }
