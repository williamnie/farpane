...
final class ViewerPasteboardProductCompositionContractTests: XCTestCase {
    ...
    func testAppKitOwnerIsTheOnlySwiftPasteboardBoundary() throws {
        ...
        XCTAssertTrue(owner.contains("ViewerClipboardImagePolicy.accepts(payload)"))
        XCTAssertTrue(owner.contains("NSPasteboard.PasteboardType(\"public.svg-image\")"))
        XCTAssertTrue(owner.contains("NSBitmapImageRep(data: tiff)"))
        XCTAssertTrue(owner.contains("CGImageSourceCreateWithData(data as CFData, options)"))
        XCTAssertTrue(owner.contains("kCGImageSourceShouldCache: false"))
        XCTAssertTrue(owner.contains("CGImageSourceGetCount(source) == 1"))
        XCTAssertTrue(owner.contains("representation(using: .png"))
        XCTAssertTrue(owner.contains("ViewerClipboardContentItemSelection.select("))
        XCTAssertTrue(owner.contains("acceptedTypeIdentifiers: Self.imagePasteboardTypes"))
        XCTAssertTrue(owner.contains("from item: NSPasteboardItem"))
        XCTAssertTrue(owner.contains("item.data(forType: type)"))
    ...
    func testProductExplicitlyEnablesBothDirectionsAcrossRecovery() throws {
        ...
        XCTAssertEqual(app.components(separatedBy: "receiveClipboardImage: true").count - 1, 3)
        XCTAssertEqual(app.components(separatedBy: "sendClipboardImage: true").count - 1, 3)
        ...
        XCTAssertTrue(app.contains("receiveImageEnabled: configuration.receiveClipboardImage"))
        XCTAssertTrue(app.contains("sendImageEnabled: configuration.sendClipboardImage"))
        ...
        XCTAssertTrue(app.contains("onClipboardImage: { [weak self] payload in"))
    ...
    func testStreamingAndTeardownBoundPasteboardLifecycle() throws {
        ...
        XCTAssertTrue(app.contains("viewerPasteboardOwner.receiveRemoteImage("))
