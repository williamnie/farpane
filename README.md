...
- 音频和文件传输产品能力默认关闭；剪贴板已提供逐类型、逐方向显式开关。Host ABI v16
  已建立独立文件传输能力门，但 App/Agent 与 Viewer 文件界面尚未开放。
...
- Audio and file transfer remain product-default-off; clipboard now has explicit per-type,
  per-direction switches. Host ABI v16 has a dedicated file-transfer gate, but App/Agent and
  Viewer file UI do not enable it.
