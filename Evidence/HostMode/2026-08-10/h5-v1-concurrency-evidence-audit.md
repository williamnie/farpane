...
- Froze the minimum timestamped cross-process lifecycle evidence contract before adding a writer or five-result manifest validator. H5.3w implemented the strict writer/schema, H5.3x connected App process start/termination, H5.3y connected exact Viewer session/Core/App teardown edges, H5.3aa connected HostAgent process start/termination, H5.3ab connected the first post-startup ready-zero HostAgent self-observation, and H5.3ac added the HostAgent-local transition normalizer. H5.3z verified that App-side Host composition must remain fail closed until the versioned XPC identity binds the exact HostAgent PID and process-start identity; lossless continuous Host publication, actual Viewer auto-recovery and the validator remain open.
...
H5.3w provides a default-off timestamped Host/Viewer lifecycle writer with separate per-process files, domain-separated identity digests, strict role/event shapes and terminal lifecycle ordering. H5.3x gives the App process one best-effort owner that derives exact kernel process-start identity plus packaged build identity, then records start/termination. H5.3y adds serialized Viewer epochs and maps only real Core streaming/terminal plus App teardown edges. H5.3aa reuses the same role-bound owner inside the HostAgent product lifetime: it consumes the already-preflighted Agent build ID, records start before `HostAgentProcessRunner.run`, and records termination through a defer after every sanitized run result. H5.3ab retains the exact lease boot/build/config identity inside the running lifetime and, only after XPC listener activation, records `readyZeroInbound` when the current sanitized snapshot is exactly ready/registered with zero authenticated connections and no active session. H5.3ac routes that record through an owner-maintained normalizer: strict source generations and one immutable Host/boot/config/build scope reject stale, duplicate, concurrent same-generation and identity-drift inputs; disconnected/recovered generations are derived rather than caller supplied. The current App still closes the session on terminal Core state, so same-epoch recovery is supported by the owner but not produced by current product behavior. Continuous Agent Host transitions still lack a lossless publication seam, and all App Host observations remain unconnected. The repository still contains no five-scenario validator or passing V1 concurrency result.
...
After H5.3ac, the audit emits `status=host-agent-transition-normalizer-implemented` only while the writer, App process/Viewer composition, HostAgent process lifetime, initial readiness composition, Host transition normalizer, available Host authority, exact missing App-visible Agent process identity and remaining gaps match the repository. It intentionally claims neither lossless continuous Agent/App Host observation, Viewer recovery nor a V1 matrix pass.

The next implementation boundary is an authoritative lossless HostAgent-local state publication seam feeding the normalizer, without polling or event-after-read inference. A later versioned, fail-closed HostAgent process-identity XPC contract remains mandatory before App Host observation can bind exact Agent PID/start identity to the same validated peer. The five-result manifest validator comes only after App and HostAgent writers are connected to authoritative lifecycle events. Installed App/Agent and two-machine execution remain a later manual checkpoint.

H5.3z verification completed with focused audit 1/1, full ScriptTests 96/96, Python compilation and `git diff --check`. H5.3aa added HostAgent process-lifetime composition, H5.3ab added initial ready evidence, and H5.3ac advances the audit to 38/38 evidence checks and 82/82 source anchors. H5.3ac fresh verification is recorded in `h5-v1-concurrency-host-transition-normalizer.md`.
