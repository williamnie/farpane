...
After H5.3ad, the audit emits `status=host-agent-continuous-observation-implemented` only while the writer, App process/Viewer composition, HostAgent process lifetime, Host transition normalizer, bounded continuous observation ingress, available Host authority, exact missing App-visible Agent process identity and remaining gaps match the repository. It intentionally claims neither App Host observation, Viewer recovery nor a V1 matrix pass.

The next contract boundary is the versioned, fail-closed HostAgent process-identity XPC extension required before App Host observation can bind exact Agent PID/start identity to the same validated peer. The five-result manifest validator comes only after App and HostAgent writers are connected to authoritative lifecycle events. Installed App/Agent and two-machine execution remain a later manual checkpoint.

H5.3z verification completed with focused audit 1/1, full ScriptTests 96/96, Python compilation and `git diff --check`. H5.3aa added HostAgent process-lifetime composition, H5.3ab added initial ready evidence, H5.3ac added transition normalization, and H5.3ad advances the audit to 40/40 evidence checks and 89/89 source anchors. H5.3ad fresh verification is recorded in `h5-v1-concurrency-host-continuous-observation.md`.
