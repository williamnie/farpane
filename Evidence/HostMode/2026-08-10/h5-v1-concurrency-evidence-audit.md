...
After H5.3ag, the audit emits `status=application-host-observation-composed` only while the writer, App process/Viewer composition, HostAgent process lifetime, Host transition normalizer, bounded continuous observation ingress, exact XPC v2 Agent identity and coherent App Host observation composition match the repository. It intentionally claims neither Viewer automatic recovery nor a V1 matrix pass.

The next implementation boundary is `viewer-automatic-recovery-composition`: current evidence normalization already requires a same-epoch authoritative Core streaming edge, but product teardown does not yet provide an automatic reconnect lifecycle. The five-result manifest validator comes only after Viewer recovery is connected. Installed App/Agent and two-machine execution remain a later manual checkpoint.
