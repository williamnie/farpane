...
- Froze the minimum timestamped cross-process lifecycle evidence contract before adding a writer or five-result manifest validator. H5.3w implemented the strict writer/schema, H5.3x connected App process start/termination, H5.3y connected exact Viewer session/Core/App teardown edges, H5.3aa connected HostAgent process start/termination, H5.3ab connected the first post-startup ready-zero HostAgent self-observation, H5.3ac added the HostAgent-local transition normalizer, and H5.3ad connected accepted session edges plus accepted snapshot publications through one bounded generation ingress. H5.3z verified that App-side Host composition must remain fail closed until the versioned XPC identity binds the exact HostAgent PID and process-start identity; actual Viewer auto-recovery and the validator remain open.
...
H5.3ae now freezes the missing contract as a rerunnable audit. It requires an incompatible exact-key schema/wire version 2 handshake identity containing build, Host instance, Agent boot, exact Agent PID and the existing domain-separated process-start SHA-256. HostAgent is the sole authority: one `getpid()` plus matching `PROC_PIDTBSDINFO` before immutable XPC identity publication. App may accept the identity only from a compatible v2 handshake, pin all five fields through snapshot/command traffic and compare all five across reconnect. Snapshot must not redefine identity, raw process-start data must not cross XPC, and schema v1 cannot serve as a Host lifecycle evidence fallback. Shared XPC implementation remains the next step, so product composition is still fail closed.
...
After H5.3ae, the audit emits `status=host-agent-process-identity-xpc-contract-frozen` only while the writer, App process/Viewer composition, HostAgent process lifetime, Host transition normalizer, bounded continuous observation ingress, available Host authority, exact missing App-visible Agent process identity, frozen v2 contract and remaining gaps match the repository. It intentionally claims neither shared XPC v2 implementation, App Host observation, Viewer recovery nor a V1 matrix pass.

The next implementation boundary is `host-agent-xpc-wire-identity-v2`: apply the frozen versioned, fail-closed HostAgent process-identity XPC extension before App Host observation can bind exact Agent PID/start identity to the same validated peer. The five-result manifest validator comes only after App and HostAgent writers are connected to authoritative lifecycle events. Installed App/Agent and two-machine execution remain a later manual checkpoint.

H5.3z verification completed with focused audit 1/1, full ScriptTests 96/96, Python compilation and `git diff --check`. H5.3aa added HostAgent process-lifetime composition, H5.3ab added initial ready evidence, H5.3ac added transition normalization, H5.3ad added continuous observation, and H5.3ae adds the focused version-2 contract audit plus frozen evidence. H5.3ae fresh verification is recorded in `h5-v1-concurrency-agent-process-identity-xpc-contract.md`.
